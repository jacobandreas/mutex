...
class EncDec(nn.Module):
    def __init__(self, 
        ...
        self.nll = nn.CrossEntropyLoss(ignore_index=vocab.pad())
        self.nll_wr = nn.CrossEntropyLoss(ignore_index=vocab.pad(), reduction='none')
        self.dim = dim
    ...
    def forward(self, inp, out, per_instance=False):
        ...
        if per_instance:
            ...
            output = dec.transpose(0,1).transpose(1,2) #TODO: too much transpose
            loss = self.nll_wr(output,out_tgt).sum(dim=-1)
        else:
            out_tgt = out[1:, :].view(-1)
            dec = dec.view(-1, len(self.vocab))
